...
The answer is **Apache Spark**.

Spark powers modern data platforms like:
...
- ✔ Optimizes execution using DAG (Directed Acyclic Graph)  
- ✔ Minimizes disk I/O  
...
- Spark keeps intermediate results in memory  
- Reduces disk I/O  
- Improves speed drastically  

### 3️⃣ Fault Tolerance

- If a machine fails: Spark **automatically recomputes lost data**  
- Jobs continue running  
- Enabled by **RDD lineage**  
...
> It reads, processes, and writes data back. Storage is handled by systems like ADLS or S3.

---

## 📦 8️⃣ Spark in Cloud Platforms

- **Azure Databricks:** Spark = processing engine, Databricks = managed infra, ADLS = storage  
- **Microsoft Fabric:** Spark = compute, OneLake = storage, managed platform  
...
|------|------------|---------|
| Batch Processing | Processes historical data | Daily sales summary |
| Streaming Processing | Processes real-time data | IoT sensor monitoring |
...
> These form the **Spark Architecture**, which we cover in the next chapter.
